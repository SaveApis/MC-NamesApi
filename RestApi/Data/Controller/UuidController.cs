...
public class UuidController : ControllerBase
{
    ...
    public UuidController(DataContext context)
    {
        _context = context;
    }
    ...
    public async Task<ActionResult<IRestResult<Guid>>> ByCurrentName(string name, [FromQuery] string lang = "en")
    {
        ...
        if (nameModel is null)
            return Ok(IRestResult.Create<Guid>(true, "Der Spieler wurde nicht gefunden."));

        if (!await _context.CheckAgreement(nameModel.Uuid))
            return Ok(IRestResult.Create<Guid>(true,
        ...
        return Ok(IRestResult.Create(false, "Die Uuid wurde erfolgreich ausgelesen.", nameModel.Uuid));
    }
    ...
    public async Task<ActionResult<IRestResult<Guid>>> ByHistoryName(string historyName, [FromQuery] string lang = "en")
    {
        ...
        return Ok(IRestResult.Create(false, "Die Uuid wurde erfolgreich ausgelesen.", historyModel.Player.Uuid));
    }
}